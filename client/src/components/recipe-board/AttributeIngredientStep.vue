...
// props.ingredients가 변경될 때 ingredients 배열을 업데이트
watch(() => props.ingredients, (newIngredients) => {
    ingredients.value = [...newIngredients]; // 깊은 복사로 배열을 새로 할당
    console.log("변경된 ingredients 배열", ingredients.value);
});

// props.manual_step이 변경될 때 steps 배열을 업데이트
watch(() => props.manual_step, (newSteps) => {
    steps.value = [...newSteps]; // 깊은 복사로 배열을 새로 할당
    console.log("변경된 manual_step 배열:", steps.value);
});
