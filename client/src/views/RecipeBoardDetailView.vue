<template>
      ...
      <MainBoard :cur="'recipe-board'">
        ...
        <div class="profile-container" style="justify-content: center; align-items: center;">
          <div class="profile-image">
            <img :src="defaultImage" alt="" style="width: 100px; height: 100px; border-radius: 50px;">
          </div>
          <div style="display: flex;">
            <div style="margin-top: 10px; font-size: 3rem;">
              닉네임{{menuInfo.user_id}}
            </div>
            <div>
              <img :src="defaultImage" alt="배지 이미지" style="width: 20px; height: 20px ;">
        ...
        <div class="favorite-button-container">
          ...
          <!-- 대댓글 컴포넌트 -->
          <WriteRecommentInput>
            :recomments="recommentInfo"
            @add-recomment="hadleAddRecomment"
              
          </WriteRecommentInput>

          <!-- 댓글 입력 -->
          <div class="comment-input" style="width: 80%; margin-top: 3rem; padding:  0 0 3rem 0;">
              <input v-model="newComment" placeholder="댓글을 입력하세요" style="width: 92%; border: 1px solid gray; height: 5rem; padding: 2rem;"/>
              <button @click="submitComment" style="height: 5rem; background-color:#3E3E3E ; color: white; border-style: none; padding: 0 2rem;"> 등록</button>
          </div>
  ...
  try {
    ...
    const commentsResponse = (await axios.get(`/api/recipe-board/${route.params.board_id}/comments`)).data;
