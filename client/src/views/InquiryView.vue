      ...
      <div class="wrapper">
        ...
        <div class="L3">
          <div class="list" v-for="inquiry in pageInquiries" :key="inquiry.inquiryId">
            <a href="" @click="toInquiry(inquiry.inquiryId)">{{ inquiry.inquiryId }}</a>
            <div class="title">
              <a href="" @click="toInquiry(inquiry.inquiryId)">{{ inquiry.inquiryTitle }}</a>
...
<script setup>
  ...
  onMounted(async () => {
    const response = await fetch("http://localhost:8080/inquiry/get-only");
  ...
  const setinquiry = (data) => {
    for (let i=0; i<data.length; i++)
      data[i].inquiryCreatedAt = data[i].inquiryCreatedAt.slice(0, 3).join('-');
    inquiry.value =data;
    inquiry.value = inquiry.value.reverse();
